...
{
    int i62 = blockIdx.x;
    int i63 = (threadIdx.x % (256));
    
    if (threadIdx.x >= 256) {
        return;
    }

    int thread_gid = i62 * 256 + i63;
    ...
    // 当前行的起始和结束位置
    unsigned int nz_start = row_off[thread_gid];
    unsigned int nz_end = row_off[thread_gid + 1];

    float temp_result = 0;

    for (unsigned int nz = nz_start; nz < nz_end; nz++)
    {
        temp_result = temp_result + values[nz] * vector[col_idx[nz]];
    }

    res[thread_gid] = temp_result;
}
...
float *driver(float *values, unsigned int *col_idx, unsigned int *row_off, unsigned int *block_first_row_index, float *x, float *y, unsigned int m, unsigned int n, unsigned int nnz, unsigned int block_num, unsigned int repeat_num, float& exe_time, float& exe_gflops)
{
    ...
    {
        spmv<<<(m+255)/256, 256>>>(dvalues, dcol_idx, drow_off, dblock_first_row_index, dvect, dres, m, n, nnz, block_num);
        cudaDeviceSynchronize();
    }
...
int main(int argc, char ** argv)
{
    ...
    printf("TACO_CSR: time=%fms, gflops=%f\n", exe_time, exe_gflops);
