...
string code_of_template_kernal(shared_memory_template_warp_compress_t *output_template, unsigned long dense_block_id)
{
    ...
    // cout << "output_template->thread_num_in_block:" << output_template->thread_num_in_block << " output_template->thread_num_of_row_reduce:" << output_template->thread_num_of_row_reduce << endl;
    ...
    {
        return_str = return_str + "\n" + code_of_data_type(find_most_suitable_data_type(output_template->kernal_first_row_index)) + "  kernal_first_row_index = " + to_string(output_template->kernal_first_row_index) + ";\n\n";
    }

    if (output_template->kernal_first_col_index != 0)
    {
        return_str = return_str + "\n" + code_of_data_type(find_most_suitable_data_type(output_template->kernal_first_col_index)) + "  kernal_first_col_index = " + to_string(output_template->kernal_first_col_index) + ";\n\n";
    }
    ...
    {
        has_block_level_shared_mem = true;
        return_str = return_str + "__shared__ " + code_of_data_type(output_template->data_type_of_block_begin_thread_index_offset) + " this_block_begin_thread_index_offset_shared[1];\n";
        return_str = return_str + "__shared__ " + code_of_data_type(output_template->data_type_of_block_begin_thread_index_offset) + " next_block_begin_thread_index_offset_shared[1];\n";
        shared_memory_item_num = shared_memory_item_num + 2;
    }
    ...
    for (unsigned long block_id = 0; block_id < output_template->size_of_block_nz_begin_offset; block_id++)
    {
        ...
        // cout << "output_template->size_of_block_begin_thread_index_offset:" << output_template->size_of_block_begin_thread_index_offset << endl;
        // cout << "output_template->size_of_block_nz_begin_offset:" << output_template->size_of_block_nz_begin_offset << endl;
        assert(block_id + 1 < output_template->size_of_block_begin_thread_index_offset);
        unsigned long this_block_first_thread_index = read_from_array_with_data_type(output_template->block_begin_thread_index_offset, output_template->data_type_of_block_begin_thread_index_offset, block_id);
        unsigned long next_block_first_thread_index = read_from_array_with_data_type(output_template->block_begin_thread_index_offset, output_template->data_type_of_block_begin_thread_index_offset, block_id + 1);
    ...
    if (output_template->block_nz_begin_offset_compress == NONE_COMPRESS || output_template->block_begin_thread_index_offset_compress == NONE_COMPRESS || output_template->block_first_row_index_compress == NONE_COMPRESS)
    {
        need_block_level_shared_init = true;
    }

    bool need_next_block_begin_thread_index_offset = true;
    // 如果没有thread层次的循环，并且每个thread粒度的块只有一个非零元,就不需要block的thread索引上界
    ...
    {
        // 只有thread有多个非零元也可以通过block_begin_thread_index_offset_compress线性压缩来消除对于next_block_begin_thread_index_offset要求
        need_next_block_begin_thread_index_offset = false;
    }

    bool need_this_block_begin_thread_index_offset = true;
    // 如果查看是否需要块的第一个线程粒度的块的索引，这个索引主要用来计算每个block的thread的数量已经快内的循环，在特定的场景下不需要
    if (output_template->thread_block_size_in_block_compress == CONSTANT_COMPRESS && ((constant_compress_t *)output_template->thread_block_size_in_block_compress_meta)->constant == 1)
    {
        if (output_template->block_begin_thread_index_offset_compress == LINEAR_COMPRESS && ((linear_compress_t *)output_template->block_begin_thread_index_offset_compress_meta)->coefficient == output_template->thread_num_in_block)
        {
            need_this_block_begin_thread_index_offset = false;
        }
    }
    else
    {
    ...
    // 查看是否需要下一个block的首行索引，当线性压缩，每一个block的行数量和thread的数量相同的时候就不需要了
    ...
    if (need_this_block_begin_thread_index_offset == true)
    {
        return_str = return_str + code_of_data_type(output_template->data_type_of_block_begin_thread_index_offset) + " this_block_begin_thread_index_offset;\n";
    }

    // 下一个块的thread起始索引，如果没有遍历、并且每个线程粒度的块中非零元的数量等于1，就不需要这个索引
    if (need_next_block_begin_thread_index_offset == true)
    {
        return_str = return_str + code_of_data_type(output_template->data_type_of_block_begin_thread_index_offset) + " next_block_begin_thread_index_offset;\n";
    }
    ...
    // 每个block的线程粒度的块的大小
    return_str = return_str + code_of_data_type(output_template->data_type_of_thread_block_size_in_block) + " this_block_thread_block_size;\n\n";
    ...
    // 如果是每个线程的非零元数量多于一个，就需要记录每个block的线程粒度的块的数量
    if (output_template->thread_block_size_in_block_compress == CONSTANT_COMPRESS && ((constant_compress_t *)output_template->thread_block_size_in_block_compress_meta)->constant == 1)
    {
    }
    else
    {
        return_str = return_str + code_of_data_type(output_template->data_type_of_block_begin_thread_index_offset) + " thread_block_num_in_this_block;\n\n";
    }

    // 如果需要线程块粒度的共享内存初始化，就需要一个全局同步
    if (need_block_level_shared_init == true)
    {
        ...
        {
            assert(output_template->block_begin_thread_index_offset != NULL);
            return_str = return_str + "this_block_begin_thread_index_offset_shared[0] = block_begin_thread_index_offset[block_level_block_id];\n";
            if (need_next_block_begin_thread_index_offset == true)
            {
                return_str = return_str + "next_block_begin_thread_index_offset_shared[0] = block_begin_thread_index_offset[block_level_block_id + 1];\n";
            }
            // return_str = return_str + "next_block_begin_thread_index_offset_shared[0] = block_begin_thread_index_offset[block_level_block_id + 1];\n";
        }
    ...
    if (output_template->block_begin_thread_index_offset_compress == NONE_COMPRESS)
    {
        ...
        if (need_this_block_begin_thread_index_offset == true)
        {
            return_str = return_str + "this_block_begin_thread_index_offset = this_block_begin_thread_index_offset_shared[0];\n";
        }

        if (need_next_block_begin_thread_index_offset == true)
        {
            return_str = return_str + "next_block_begin_thread_index_offset = next_block_begin_thread_index_offset_shared[0];\n";
        }
    }
    else if (output_template->block_begin_thread_index_offset_compress == LINEAR_COMPRESS)
    {
        ...
        if (need_this_block_begin_thread_index_offset == true)
        {
            return_str = return_str + code_of_arr_read(compressor, "this_block_begin_thread_index_offset", "block_level_block_id") + ";\n";
        }

        // 这里做一个加法来获得下一位
        if (need_next_block_begin_thread_index_offset == true)
        {
            if (need_this_block_begin_thread_index_offset == false)
            {
                return_str = return_str + code_of_arr_read(compressor, "next_block_begin_thread_index_offset", "(block_level_block_id + 1)") + ";\n";
            }
            else
            {
                return_str = return_str + "next_block_begin_thread_index_offset = this_block_begin_thread_index_offset + " + to_string(compressor->coefficient) + ";\n";
            }
        }
    }
    else
    ...
    {
        assert(output_template->thread_block_size_in_block != NULL);
        return_str = return_str + "this_block_thread_block_size = thread_block_size_in_block_shared[0];\n";
    }
    else if (output_template->thread_block_size_in_block_compress == CONSTANT_COMPRESS)
    {
        ...
        return_str = return_str + code_of_arr_read(compressor, "this_block_thread_block_size", "block_level_block_id") + ";\n";
    }
    else if (output_template->thread_block_size_in_block_compress == BRANCH_COMPRESS)
    {
        ...
        return_str = return_str + "\n" + code_of_arr_read(compressor, "this_block_thread_block_size", "block_level_block_id") + "\n";
    }
    else
    {
        cout << "this compress type is not support" << endl;
    ...
    {
        // 每个block块的数量可以通过两种方式计算得出，一种是用next当前首块索引相减，一种是当每个block内thread数量一样的时候，直接赋值
        if (output_template->block_begin_thread_index_offset_compress == LINEAR_COMPRESS)
        {
            // 这里说明首thread的偏移是固定步长自增的
            assert(output_template->block_begin_thread_index_offset_compress_meta != NULL);

            unsigned long thread_block_num_in_block_block = ((linear_compress_t *)output_template->block_begin_thread_index_offset_compress_meta)->coefficient;

            return_str = return_str + "thread_block_num_in_this_block = " + to_string(thread_block_num_in_block_block) + ";\n";
        }
        else
        {
            // 相减
            assert(need_next_block_begin_thread_index_offset == true);
            assert(need_this_block_begin_thread_index_offset == true);
            return_str = return_str + "thread_block_num_in_this_block = next_block_begin_thread_index_offset - this_block_begin_thread_index_offset;\n";
        }
    }
    ...
    // 用一个块内线程粒度块索引的变量名
    string var_name_of_inner_thread_level_block;

    // 遍历当前块的所有线程粒度的块，如果正好和线程块的大小一样就不需要一个for循环
    if (output_template->block_begin_thread_index_offset_compress == LINEAR_COMPRESS && ((linear_compress_t *)output_template->block_begin_thread_index_offset_compress_meta)->coefficient == output_template->thread_num_in_block)
    {
        assert(need_next_block_begin_thread_index_offset == false);
        // 不需要for循环
        return_str = return_str + "{\n";
        // 不需要初始化任何东西
        var_name_of_inner_thread_level_block = "tid_in_block";
    }
    else
    {
        // 需要的for循环
        ...
        assert(need_this_block_begin_thread_index_offset == true);
        return_str = return_str + "unsigned int" + " thread_level_block_id = this_block_begin_thread_index_offset + tid_in_block; ";

        assert(need_next_block_begin_thread_index_offset == true);
        return_str = return_str + "thread_level_block_id < next_block_begin_thread_index_offset; thread_level_block_id = thread_level_block_id + blockDim.x)\n{\n";

        // 计算块内索引
        return_str = return_str + "unsigned int" + " thread_level_block_index_inner_block = thread_level_block_id - this_block_begin_thread_index_offset;\n\n";

        var_name_of_inner_thread_level_block = "thread_level_block_index_inner_block";
    }

    // 首个非零元
    return_str = return_str + code_of_data_type(output_template->data_type_of_block_nz_begin_offset) + " global_nz_in_thread_level = this_block_first_nz_index + " + var_name_of_inner_thread_level_block + ";\n\n";

    // 补足列方向的偏移量的代码
    string code_of_col_index_offset = "";

    if (output_template->kernal_first_col_index != 0)
    {
        code_of_col_index_offset = "kernal_first_col_index + ";
    }
    ...
    {
        // 声明一个数存储中间结果
        return_str = return_str + code_of_data_type(output_template->data_type_of_val_arr) + " thread_tmp_result;\n";
        // 不循环的版本
        return_str = return_str + "{\n";
        // 直接给结果算完之后赋值
        return_str = return_str + "thread_tmp_result = val_arr[global_nz_in_thread_level] * __ldg(&(device_x_arr[" + code_of_col_index_offset + "col_index_arr[global_nz_in_thread_level]]));\n";
    }
    else
    {
        ...
        // for循环的版本
        return_str = return_str + "for(";
        return_str = return_str + "unsigned int" + " nz_index_in_thread_level = 0; nz_index_in_thread_level < this_block_thread_block_size; nz_index_in_thread_level++)\n{\n";
        return_str = return_str + "thread_tmp_result = thread_tmp_result + val_arr[global_nz_in_thread_level] * __ldg(&(device_x_arr[" + code_of_col_index_offset + "col_index_arr[global_nz_in_thread_level]]));\n";
        // 自增
        return_str = return_str + "global_nz_in_thread_level = global_nz_in_thread_level + thread_block_num_in_this_block;\n";
    }
    ...
    return_str = return_str = return_str + "thread_tmp_result_inner_block[" + var_name_of_inner_thread_level_block + "] = thread_tmp_result;\n";
    // return_str = return_str + "thread_tmp_result_inner_block[" + var_name_of_inner_thread_level_block + "] = thread_tmp_result;\n";
    return_str = return_str + "}\n";
    ...
    if (output_template->thread_num_of_row_reduce == 1)
    {
        ...
        if (output_template->block_first_row_index_compress == LINEAR_COMPRESS && ((linear_compress_t *)output_template->block_first_row_index_compress_meta)->coefficient == output_template->thread_num_in_block)
        {
            ...
            // 根据没有没有压缩视图的row padding来决定是不是在规约时采用if语句
            if (output_template->matrix->block_coor_table.item_arr[dense_block_id]->max_dense_row_index == output_template->matrix->block_coor_table.item_arr[dense_block_id]->compressed_block_ptr->read_index[0]->max_row_index)
            {
                return_str = return_str + "{\n";
            }
            else
            {
                return_str = return_str + "if (row_id < " + to_string(output_template->effective_row_num) + ")\n{";
            }
        }
        else
        {
            ...
            return_str = return_str + "unsigned int" + " row_id = this_block_first_row_index + tid_in_block; row_id < next_block_first_row_index && row_id < " + to_string(output_template->effective_row_num) + "; row_id = row_id + blockDim.x)\n{\n";
        }
    ...
    else
    {
        ...
        if (output_template->block_first_row_index_compress == LINEAR_COMPRESS && ((linear_compress_t *)output_template->block_first_row_index_compress_meta)->coefficient == (output_template->thread_num_in_block / output_template->thread_num_of_row_reduce))
        {
            ...
            // 行号
            return_str = return_str + code_of_data_type(output_template->data_type_of_block_first_row_index) + " row_id = this_block_first_row_index + tid_in_block / active_row_reduce_thread_num;\n";
        }
        else
        {
            // 需要for循环，首先计算遍历的步长
        ...
        return_str = return_str + "for(";
        return_str = return_str + "unsigned int" + " cur_active_row_reduce_thread_num = active_row_reduce_thread_num; cur_active_row_reduce_thread_num >= 1; cur_active_row_reduce_thread_num = cur_active_row_reduce_thread_num / 2)\n{\n";
    ...
    if (output_template->thread_num_of_row_reduce != 1)
    {
        // 并且这个线程负责的行要判断自己的行号是不是有效行号
        return_str = return_str + "if (" + tid_in_row_var_name + " == 0 && " + row_id_var_name + " < " + to_string(output_template->effective_row_num) + ")\n{\n";
    }
    else
    {
        return_str = return_str + "if (" + row_id_var_name + " < " + to_string(output_template->effective_row_num) + ")\n{\n";
    }
...
string code_of_write_template_data_to_gpu(shared_memory_template_warp_compress_t *output_template, unsigned long dense_block_id, bool force_not_share_global_sort_index)
{
...
bool compress_block_begin_thread_index_offset(shared_memory_template_warp_compress_t *output_template, bool need_check, arr_compress_type type)
{
...
bool compress_thread_block_size_in_block(shared_memory_template_warp_compress_t *output_template, bool need_check, arr_compress_type type)
{
...
bool compress_block_nz_begin_offset(shared_memory_template_warp_compress_t *output_template, bool need_check, arr_compress_type type)
{
...
bool compress_block_first_row_index(shared_memory_template_warp_compress_t *output_template, bool need_check, arr_compress_type type)
{
    // 基本只能线性压缩
...
bool compress_row_offset_in_thread_tmp_result(shared_memory_template_warp_compress_t *output_template, bool need_check, arr_compress_type type)
{
...
bool set_row_reduce_thread_num(shared_memory_template_warp_compress_t *output_template, unsigned long row_reduce_thread_num)
{
    ...
    else if (row_reduce_thread_num == get_config()["HALF_MAX_ROW_REDUCE_THREAD"].as_integer())
    {
        ...
        // cout << output_template->size_of_block_first_row_index << " " << output_template->size_of_block_begin_thread_index_offset << endl;
        // row的偏移量和thread的偏移量都是CSR压缩的
        assert(output_template->size_of_block_first_row_index == output_template->size_of_block_begin_thread_index_offset);
        // assert(output_template->size_of_);
        ...
        for (unsigned long i = 0; i < output_template->size_of_block_first_row_index - 1; i++)
        {
            ...
            if (block_row_num != 1)
            {
                cout << "block row num is not equal to 1, block_row_num:" << block_row_num << " i:" << i << endl;
                return false;
            }

            // 查看当前线程粒度的块的数量，需要和线程块中线程的数量保持完全一致（其实两倍也可以，不做进一步考虑）
            unsigned long block_thread_num = read_from_array_with_data_type(output_template->block_begin_thread_index_offset, output_template->data_type_of_block_begin_thread_index_offset, i + 1) - read_from_array_with_data_type(output_template->block_begin_thread_index_offset, output_template->data_type_of_block_begin_thread_index_offset, i);

            if (block_thread_num != output_template->thread_num_in_block)
            {
                cout << "thread level block num is not equal to blockDim.x, block_thread_num:" << block_thread_num << ", output_template->thread_num_in_block:" << output_template->thread_num_in_block << endl;
                return false;
            }
        }
    ...
    else
    {
        ...
        // 中间结果数组存储了所有这些信息
        // cout << "output_template->size_of_row_offset_in_thread_tmp_result:" << output_template->size_of_row_offset_in_thread_tmp_result << endl;
        // cout << "output_template->dense_block_index:" << output_template->dense_block_index << endl;
        // cout << output_template->matrix->block_coor_table.item_arr[3]->min_dense_row_index << " " << output_template->matrix->block_coor_table.item_arr[3]->max_dense_row_index << endl;
        // cout << "output_template->matrix->block_coor_table.item_arr[output_template->dense_block_index]->max_dense_row_index + output_template->matrix->block_coor_table.item_arr[output_template->dense_block_index]->min_dense_row_index + 2:" << output_template->matrix->block_coor_table.item_arr[output_template->dense_block_index]->max_dense_row_index + output_template->matrix->block_coor_table.item_arr[output_template->dense_block_index]->min_dense_row_index + 2 << endl;
        assert(output_template->size_of_row_offset_in_thread_tmp_result == output_template->matrix->block_coor_table.item_arr[output_template->dense_block_index]->max_dense_row_index - output_template->matrix->block_coor_table.item_arr[output_template->dense_block_index]->min_dense_row_index + 2);
