...
// 实现一个ell版本的spmv
__global__ void ell_spmv(double * val_arr, unsigned int * col_index_arr, double * device_x_arr, double * device_y_arr)
{
    ...
    for(unsigned int thread_level_block_id = global_tid; thread_level_block_id < 929920; thread_level_block_id = thread_level_block_id + total_thread_num)
    {
        ...
        device_y_arr[thread_level_block_id] = thread_block_tmp_result;
    }
}

int main()
{
compressed_dense_block_0_t *dense_block_0_template_data = read_dense_block_0_from_file("/home/duzhen/spmv_builder/data_source/1610067940");

double* device_dense_0_val_arr;
unsigned int* device_dense_0_col_index_arr;

cudaMalloc(&device_dense_0_val_arr, sizeof(double) * 4649600);
cudaMemcpy(device_dense_0_val_arr, dense_block_0_template_data->dense_0_val_arr, sizeof(double) * 4649600, cudaMemcpyHostToDevice);
...
cudaMemcpy(device_y_arr, host_y_arr, sizeof(double) * 929920, cudaMemcpyHostToDevice);
...
ell_spmv<<<120, 512, 0, stream_arr[0]>>>(device_dense_0_val_arr, device_dense_0_col_index_arr, device_x_arr, device_y_arr);
...
double gflops = ((double)2.0 * 4020750 * 2000 / ((double)timeuse / 1000000)) / 1000000000;

printf("time=%fms, gflops=%f\n",timeuse / 1000.0, gflops);
cudaMemcpy(host_y_arr, device_y_arr, sizeof(double) * 929920, cudaMemcpyDeviceToHost);
print_arr_to_file_with_data_type(host_y_arr, DOUBLE, 929920, "/home/duzhen/spmv_builder/data_source/test_result_3");
